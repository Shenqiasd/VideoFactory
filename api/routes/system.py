...
_OAUTH_PLATFORM_GUIDES: dict[str, dict] = {
    ...
    "bilibili": {
        ...
        "auth_type": "oauth2",
        "scopes": "视频上传、用户信息",
        "dev_portal_url": "https://open.bilibili.com/doc/4/eaf0e2b5-bde9-b9a0-9be1-019bb455701c",
        "dev_portal_name": "哔哩哔哩开放平台",
        "env_vars": ["OAUTH_BILIBILI_CLIENT_ID", "OAUTH_BILIBILI_CLIENT_SECRET"],
        "guide_steps": [
            "前往哔哩哔哩开放平台 (open.bilibili.com) 注册开发者账号并完成实名认证",
            "在「开平管理中心」(openhome.bilibili.com) 创建应用，选择「网站应用」类型",
            "在应用设置中配置「授权回调域」（即回调地址的域名部分），注意 gourl 参数需与此域名一致",
            "获取应用的 Client ID 和 Client Secret（查看应用详情获取）",
            "设置环境变量 OAUTH_BILIBILI_CLIENT_ID 和 OAUTH_BILIBILI_CLIENT_SECRET",
        ],
        "notes": "B站授权页已于 2025 年 3 月更新（新地址: account.bilibili.com/pc/account-pc/auth/oauth），旧地址 passport.bilibili.com 已停止维护。需实名认证，审核通过后才能使用。",
    },
    "tiktok": {
        "name": "TikTok",
        "icon": "music",
    ...
    "douyin": {
        ...
        "dev_portal_url": "https://open.douyin.com/",
        "dev_portal_name": "抖音开放平台",
        "env_vars": ["OAUTH_DOUYIN_CLIENT_ID", "OAUTH_DOUYIN_CLIENT_SECRET"],
        "guide_steps": [
            "前往抖音开放平台 (open.douyin.com) 使用企业资质注册开发者账号并完成实名认证",
            "在控制台创建移动应用或网站应用，获取 Client Key 和 Client Secret",
            "在「权限管理」中申请所需 scope：user_info（用户信息）、video.create（视频创建）、video.data（视频数据）",
            "在应用详情中配置回调地址：{callback_url}/api/oauth/callback/douyin",
            "授权链接格式：https://open.douyin.com/platform/oauth/connect?client_key=YOUR_KEY&response_type=code&scope=user_info&redirect_uri=YOUR_URI",
        ],
        "notes": "抖音使用 client_key（非 client_id），系统已自动映射。支持两种 token：应用级 client_token（无需用户授权）和用户级 access_token（需用户授权）。发布视频需用户级 token。需企业资质。",
    },
    "facebook": {
        "name": "Facebook",
        "icon": "facebook",
    ...
    "kwai": {
        ...
        "auth_method": "OAuth 2.0",
        "auth_type": "oauth2",
        "scopes": "user_info, user_video_publish, user_video_info, user_video_delete",
        ...
        "guide_steps": [
            ...
            "在「应用管理」中创建应用，获取 App ID（app_id）和 App Secret（app_secret）",
            "在应用中配置回调地址：{callback_url}/api/oauth/callback/kwai",
            "申请所需 scope 权限：user_info、user_video_publish、user_video_info",
            "授权链接格式：https://open.kuaishou.com/oauth2/authorize?app_id=YOUR_ID&scope=user_info,user_video_publish&response_type=code&redirect_uri=YOUR_URI",
        ],
        "notes": "快手使用 app_id（非 client_id），系统已自动映射。视频发布流程：发起上传→上传视频→发布视频，需按顺序调用三个接口。需企业资质。",
    },
    "xiaohongshu": {
        "name": "小红书",
        "icon": "book-open",
        ...
        "auth_type": "cookie",
        "scopes": "笔记发布（通过浏览器 Cookie 模拟操作）",
        "dev_portal_url": "https://www.xiaohongshu.com/",
        "dev_portal_name": "小红书",
        "env_vars": [],
        "guide_steps": [
            "小红书开放平台 (open.xiaohongshu.com) 仅对企业开发者开放，个人用户无法申请视频发布 API",
            "因此采用 Cookie 登录方式：在浏览器中登录小红书网页版 (www.xiaohongshu.com)",
            "使用浏览器开发者工具（F12）→ Application → Cookies 复制登录 Cookie",
            "或安装浏览器扩展（如 EditThisCookie）导出 Cookie JSON",
            "在下方「Cookie 登录」中粘贴 Cookie 完成绑定（关键字段：web_session、a1）",
        ],
        "notes": "Cookie 登录方式需要定期更新（Cookie 有效期约 30 天）。建议使用浏览器扩展自动导出。小红书开放 API 仅限企业合作伙伴。",
    },
    "weixin_sph": {
        "name": "微信视频号",
        "icon": "message-circle",
        ...
        "auth_method": "Cookie 登录",
        "auth_type": "cookie",
        "scopes": "视频发布（通过浏览器 Cookie 模拟操作）",
        "dev_portal_url": "https://channels.weixin.qq.com/",
        "dev_portal_name": "微信视频号",
        "env_vars": [],
        "guide_steps": [
            "微信视频号目前没有公开的视频发布 API，仅对少数合作伙伴开放",
            "因此采用 Cookie 登录方式：在浏览器中打开视频号创作者中心 (channels.weixin.qq.com)",
            "使用微信扫码登录后，通过开发者工具（F12）→ Application → Cookies 复制 Cookie",
            "或使用浏览器扩展导出 Cookie JSON",
            "在下方「Cookie 登录」中粘贴 Cookie 完成绑定",
        ],
        "notes": "Cookie 有效期较短（约 24 小时），需要频繁更新。未来微信可能开放正式 API，届时会自动升级。",
    },
    "weixin_gzh": {
        "name": "微信公众号",
        "icon": "newspaper",
        ...
        "auth_method": "Cookie 登录",
        "auth_type": "cookie",
        "scopes": "图文/视频发布（通过浏览器 Cookie 模拟操作）",
        "dev_portal_url": "https://mp.weixin.qq.com/",
        "dev_portal_name": "微信公众平台",
        "env_vars": [],
        "guide_steps": [
            "微信公众号虽有 OAuth 和素材管理 API，但视频发布 API 仅限认证服务号且权限受限",
            "因此采用 Cookie 登录方式：在浏览器中登录微信公众平台 (mp.weixin.qq.com)",
            "使用开发者工具（F12）→ Application → Cookies 复制登录后的 Cookie",
            "或使用浏览器扩展导出 Cookie JSON",
            "在下方「Cookie 登录」中粘贴 Cookie 完成绑定",
        ],
        "notes": "需已认证的服务号或订阅号。Cookie 有效期较短，需定期更新。如有已认证服务号且需要 API 方式，可联系管理员配置 AppID/AppSecret。",
    },
    "threads": {
        "name": "Threads",
        "icon": "at-sign",
