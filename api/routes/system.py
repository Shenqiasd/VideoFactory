...
# ---------------------------------------------------------------------------
# OAuth 平台配置（已移除 Settings UI，仅通过环境变量配置）
# ---------------------------------------------------------------------------
# OAuth 凭证现在仅通过环境变量配置（OAUTH_{PLATFORM}_{FIELD}），
# 不再提供 Settings 页面的手动输入。
# 参见 api/server.py register_platform_services() 和
# api/routes/oauth.py ALL_PLATFORMS。
